...
async fn main() -> anyhow::Result<()> {
    ...
    // P8: same persisted settings as the GUI. Project-scoped
    // ({project}/.tracelean/ai_settings.json) wins over the home fallback.
    let project_arg = std::env::args().nth(1);
    {
        let mut settings = shared.ai_settings.lock().unwrap();
        *settings = project_arg
            .as_deref()
            .and_then(|p| tracelean_core::ai::service::load_settings_from(std::path::Path::new(p)))
            .unwrap_or_else(tracelean_core::ai::service::load_settings);
    }
