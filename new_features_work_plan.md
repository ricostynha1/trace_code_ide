...
**Done (this pass):**
- `docs/architecture.md` was carrying two concrete inaccuracies from stale content, both fixed: the tool list (named tools that don't exist anymore — `write_file`, `insert_lines`, `search_files`, `emit_command` — instead of the actual `data/tools.json` names), and the Phase Roadmap's "Remaining (Phase 4)" list, which still claimed "wire built-in agent to real AI providers" as outstanding when Bedrock/OpenRouter have been live for a while.
- Added the three subsystem sections the plan called for and that were entirely missing: **Agent Shell Sandboxing** (overlay/bwrap, allowlist/protected model, user-facing settings), **Cost Model & Context Compaction** (rules table vs. cost scan split, cache markers, the tool-token accounting fixed in #2), and **Myth** (Surface/ActionRegistry/Keymap, plus the exact coverage-gap list from #3's audit) — each written as file pointers to the actual modules, not restated prose.
- `docs/TOOL_SINGLE_SOURCE.md` proposed a "runtime validation middleware" as an alternative to a `build.rs` codegen approach; #1 implemented exactly that middleware today, so the doc had a status note added marking it done, plus an honest note on the narrower gap it does *not* close (a wrong string literal inside an `execute_*` function's own `get_str_arg` call, which schema validation of the argument object can't catch).
- `README.md`: fixed a typo, added a **GUI Keybindings (Myth)** section (previously only the TUI had a keybinding table — the GUI's modal keymap had no user-facing documentation at all, which is part of why the which-key-bar bug in #3 went unnoticed), and a **Configuration** section pointing at `.tracelean/ai_settings.json` and documenting the settings fields that aren't otherwise self-explanatory from the Settings panel alone (`shell_sandbox`, `disable_context_trimming`, `n_expected_rounds`, etc.).
- Left the doc-freshness CI check as a follow-up (see Suggested additions #5/#6) — a real idea, but a separate, standing-check deliverable rather than a one-time doc pass.
